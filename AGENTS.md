...
8. **Fase UI shadcn — UI-Only (Berlaku 2026-08-23 s/d selesai)**: Selama roadmap `docs/plan/ui-shadcn-roadmap.md` berjalan, **agen HANYA boleh mengubah tampilan/UI** di `apps/web` (`components/ui/*`, `components/layout/*`, `components/operations/*`, `components/finance/*`, `components/commercial/*`, `components/reports/*`, `app/globals.css`, `lib/utils.ts`, `components.json`). **DILARANG** mengubah `context/*`, `engine/*`, `lib/supabase*.ts`, `supabase/migrations/*`, `packages/*`, `types/*`, maupun logika bulk/invoice/freight/audit. Pelanggaran = rollback. Mobile hanya token warna (nativewind), bukan rewrite.
